...
def subBytes(state):
    return bytearray([s_box[x] for x in state])

def invSubBytes(state):
    return bytearray([inv_s_box[x] for x in state])

def shiftRows(state):
    table = [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11]
    newstate = []
    for i, x in enumerate(state):
        newstate.append(state[table[i]])
...
def keyExpansion(key):
    expandedKey = []
    expandedKey.extend(key)
    rconIteration = 1
    while len(expandedKey) < 176: # 16bits key + 10 16bits round keys
        key_chunk = expandedKey[-4:]
        if len(expandedKey)%16==0:
            key_chunk = keyExpansion_core(key_chunk, rconIteration)
            rconIteration = rconIteration + 1
        xored = fixed_xor(key_chunk, bytearray(expandedKey[-16:][:4]))
        expandedKey.extend(xored)
    return expandedKey

def keyExpansion_core(key_chunk, rcon_index):
    new_key_chunk = key_chunk
    assert len(new_key_chunk) == 4
    new_key_chunk = rotateleft(new_key_chunk)
    new_key_chunk = subBytes(new_key_chunk)
    new_key_chunk[0] = new_key_chunk[0] ^ rcon[rcon_index]
    return new_key_chunk

def rotateleft(key_chunk):
    assert len(key_chunk) == 4
    return key_chunk[1:] + [key_chunk[0]]

def aes_encrypt_block(key, msg):
    assert len(key) == 16
    assert len(msg) == 16
    state = msg
    roundkeys = keyExpansion(key)
    assert len(roundkeys) == 176
    roundkeys = groupByBlocks(roundkeys, 16)
...
def easy_aes_decrypt(key, msg):
    decipher = AES.new(key, AES.MODE_ECB)
    return decipher.decrypt(msg)

def easy_aes_encrypt(key, msg):
    decipher = AES.new(key, AES.MODE_ECB)
    return decipher.encrypt(msg)

def aes_decrypt_ecb(key, msg):
    msg = bytearray(msg)
    while len(msg)%16!=0:
        msg.append(0)
    chunks = groupByBlocks(msg, 16)
    ans = []
    for chunk in chunks:
        chunk
        chunk = bytearray(chunk)
        decrypted_chunk = aes_decrypt_block(key, chunk)
        ans.extend(decrypted_chunk)
    return bytes(ans)

def aes_encrypt_ecb(key, msg):
    msg = bytearray(msg)
    while len(msg)%16!=0:
        msg.append(0)
    chunks = groupByBlocks(msg, 16)
    ...
    for chunk in chunks:
        chunk
        chunk = bytearray(chunk)
        encrypted_chunk = aes_encrypt_block(key, chunk)
        ans.extend(encrypted_chunk)
    return bytes(ans)
...
if __name__ == "__main__":
    ...
    # Version 1
    print(easy_aes_decrypt(key, hex_data).decode())
    # Version 2
    decrypted = aes_decrypt_ecb(key, hex_data)
